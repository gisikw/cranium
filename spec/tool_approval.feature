Feature: Tool Approval
  ...
  Background:
    Given the bridge is listening on /tmp/cranium.sock
  ...
  Scenario: Tool matching an allow rule is automatically approved
    ...
    When cranium-hook requests approval for tool "Read" with path "/home/dev/file.txt"
  ...
  Scenario: Tool matching a deny rule is automatically denied
    ...
    When cranium-hook requests approval for tool "Bash" with command "rm -rf /"
  ...
  Scenario: Wildcard matching in specifiers
    ...
    When cranium-hook requests approval for tool "Read" with path "/home/dev/Projects/file.txt"
    Then the bridge responds with "allow"
  ...
  Scenario: Unmatched tool request prompts the user
    ...
    When cranium-hook requests approval for that tool
  ...
  Scenario: Thumbs-up reaction approves the tool
    ...
    Then the bridge responds to cranium-hook with "allow"

  Scenario: Thumbs-down reaction denies the tool
    Given an approval prompt is pending for tool "Bash"
    When @alice reacts with thumbs-down
    Then the bridge responds to cranium-hook with "deny"
  ...
  Scenario: Hook falls back when bridge socket is unavailable
    Given the bridge socket at /tmp/cranium.sock is not available
    When cranium-hook receives a tool approval request
