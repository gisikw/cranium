...
TOOLS='[
  ...
  {"name":"switch_room","description":"Switch the client to a different conversation room. Use when the user asks to move to another room, or when the conversation naturally shifts to a topic that belongs in a specific project room. The switch executes after the current response finishes playing. Only switch to rooms that exist — room IDs match Matrix room names (e.g., fort-nix, hearth, cranium).","inputSchema":{"type":"object","properties":{"room_id":{"type":"string","description":"The room to switch to (e.g., fort-nix, hearth, cranium)"}},"required":["room_id"]}},
