defmodule Cranium.Backend.LLM.ClaudeCodeTest do
  ...
  describe "stream_chat/2 — port-based streaming" do
    test "parses stream-json from a subprocess" do
      ...
      init_line =
        Jason.encode!(%{type: "system", subtype: "init", session_id: "test-sess-001", tools: []})
      ...
      result_line = Jason.encode!(%{type: "result", subtype: "success", result: "Hello from CC"})
      ...
      Application.put_env(
        :cranium,
        :backends,
        Keyword.put(backends, :claude_code_path, script_path)
      )
    ...
    test "handles MCP marker tool calls in stream" do
      ...
      assert_receive {:llm_tool_use, %{id: "m1", name: "show", input: %{"url" => "img.png"}}},
                     5000
  ...
  defp write_temp_script(content) do
    path = Path.join(System.tmp_dir!(), "cc_test_#{:rand.uniform(999_999)}.sh")
