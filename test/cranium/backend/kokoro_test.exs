defmodule Cranium.Backend.KokoroTest do
  use ExUnit.Case, async: true

  alias Cranium.Backend.TTS.Kokoro

  @plug_name CraniumKokoroTest
  ...
  describe "synthesize/2" do
    test "returns {:ok, audio_binary} on HTTP 200" do
      ...
      assert {:ok, ^audio} = Kokoro.synthesize("Hello world", plug: {Req.Test, @plug_name})
    end
    ...
    test "returns {:error, {:http_error, 503, _}} on non-200" do
      ...
      assert {:error, {:http_error, 503, _}} =
               Kokoro.synthesize("Hello world", plug: {Req.Test, @plug_name})
    end
    ...
    test "returns {:error, _} on connection error" do
      ...
      assert {:error, _} = Kokoro.synthesize("Hello world", plug: {Req.Test, @plug_name})
    end
  end
end
